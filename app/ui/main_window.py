...
class MainWindow(QMainWindow):
    ...
    def _build_topbar(self) -> QWidget:
        ...
        self._stop_btn.setIcon(qta.icon("fa5s.stop-circle", color=SEMANTIC["danger"]))
        self._stop_btn.setFixedHeight(32)
        self._stop_btn.setToolTip("Stop the running scan  [Esc]")
        self._stop_btn.clicked.connect(self.stop_scan)
        self._stop_btn.hide()   # only visible while a scan is running
    ...
    def stop_scan(self):
        ...
        if self._scan_worker and self._scan_worker.isRunning():
            ...
            self._status_bar.set_action("Stopping scan…", NEUTRAL["text_faint"])
            self._scan_overlay.update_progress("Stopping…", 0, 0)
            self._toast.show_message("Scan stopped.", color=NEUTRAL["text_secondary"])
    ...
    def _set_controls_enabled(self, enabled: bool):
        """
        enabled=True  → normal state (scan finished / idle)
        enabled=False → scan in progress (show Stop, hide Scan now)
        """
        self._scan_btn.setVisible(enabled)

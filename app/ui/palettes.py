...
def get_palette(key: str, mode: str = "dark") -> dict:
    """Return the full flat token dict (neutral + semantic + accent + fonts)
    for a given palette key and mode, falling back to the default if unknown."""
    accent = ACCENTS.get(key, ACCENTS[DEFAULT_PALETTE])
    
    active_neutral = DARK_NEUTRAL if mode == "dark" else LIGHT_NEUTRAL
    NEUTRAL.clear()
    NEUTRAL.update(active_neutral)
    ...
    tokens["mode"] = mode if mode in ("dark", "light") else "dark"
    return tokens
